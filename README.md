...
- **测试与 CI**：GitHub Actions 覆盖 Python 3.10/3.11/3.12；本地当前测试为 `181 passed, 3 skipped`。
...
class MyStrategy(BaseStrategy):
    ...
    def handle_bar(self, ctx, data):
        if data["close"].iloc[-1] > data["close"].rolling(20).mean().iloc[-1]:
            ctx.order_target_pct("600519", 0.95)

engine = BacktestEngine("600519", start=date(2024,1,1), end=date(2025,12,31))
results = engine.run(MyStrategy())
print(engine.report(save_chart="backtest.png"))
```

回测报告包含：
- 总/年化收益、夏普比率、最大回撤、Calmar 比率
- 交易级胜率、盈亏比、平均持仓天数
- 基准对比（默认沪深300）：Alpha、信息比率
- 月度/年度收益分布
- 权益曲线图（含回撤水下图和买卖标记）
...
# 全量新闻价值发现：识别主题、板块映射、价值评分
python -m modules.cli tool event news --json --limit 100

# 物化新闻板块因子：供评分系统和盘中监控读取
python -m modules.cli tool event news-factors --json

# 更新主题记忆：追踪 AI、能源、军工、黄金等主题热度演化
python -m modules.cli tool event topics --json

# LLM 语义评审预览：默认 dry-run，不调用外部模型
python -m modules.cli tool event llm-review --json

# 显式调用外部模型评审：需要本地配置和环境变量 API Key
...
`event report` 默认输出“热点速览”：

- 顶部汇总：扫描源数量、捕获资讯数量、高热模块、无触发阈值模块。
- 模块分组：按热点数量和热度排序，例如 `算力基础设施 · 硬件突破与政策双轮驱动`。
- 单条热点：标题、热度、来源国别、核心看点、传导路径、选中理由、A 股方向和一句话播报。
- 映射原则：优先按细粒度产业标签匹配股票，例如 `服务器电源`、`光无源器件`、`绿色算力`，避免直接套用通用模块股票池。
- 安全边界：报告只输出情报、机会和评分因子，不自动下单，不构成投资建议。
